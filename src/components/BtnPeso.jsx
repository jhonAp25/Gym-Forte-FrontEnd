...
const BtnPeso = ({peso , setPeso}) => {

    
 const changeMinus=()=>{
    setPeso(parseInt(peso) - 1 )
    console.log(peso);
   // console.log(newPeso)
    return;
 }
 ...
 const changePlus =()=>{
    ...
    return (    
        <div className='bg-yellow-500 flex rounded-full items-center justify-between border mb-3 w-1/2'>
            ...
            <div className="bg-white w-full h-full  flex rounded-full text-center py-2 flex- justify-center items-center "> <p className=" font-semibold text-xl"> {peso} </p> <p className="  font-light text-sm">Kg</p> </div>
            <AiFillCaretRight color="#fff" size={30} className=" mx-2 cursor-pointer" onClick={()=>changePlus()}  />
        </div>
    )
}

export default BtnPeso